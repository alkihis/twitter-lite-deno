import OAuth from '../../deno-oauth-1.0a/mod.ts';
...
function stringifyQuerystring(parameters: { [name: string]: string }) {
  let final = '';
...
export default class Twitter {
  ...
  async getBearerToken() {
    const auth = window.btoa(this.config.consumer_key + ':' + this.config.consumer_secret);
  ...
  async getRequestToken(twitterCallbackUrl: string) {
    ...
    if (parameters) requestData.url += '?' + stringifyQuerystring(parameters);
  ...
  _makeRequest(method: string, resource: string, parameters?: any, rawRequest = false) {
    ...
    if (parameters)
      ...
      else requestData.url += '?' + stringifyQuerystring(parameters);
  ...
  post(resource: string, body?: any) {
    ...
    if (JSON_ENDPOINTS.includes(resource)) {
      ...
      body = percentEncode(stringifyQuerystring(body));
